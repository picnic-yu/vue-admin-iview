<template>
	<div class="search" >
		...
		<Input v-model="value" :placeholder="placeholderValue" style="width: 300px" ></Input>
		<Button type="primary" icon="ios-search" class='query' @click='query'>查询</Button>
		<Button type="primary" @click='empty'>清空</Button>
		<Button type="primary" class='more hide_wrap' @click='showMore'>高级查询</Button>
		...
		mounted(){
			this.$nextTick(() => {
				document.querySelector('body').addEventListener('click', (event) => {
					...
					if( event.target.parentElement.parentElement.className.indexOf('show_wrap') > -1 ) return;
